...
function Luna:CreateWindow(WindowSettings)
    ...
    -- store values manually (IMPORTANT)
    ...
    -- color pickers (callbacks defined HERE, not with :Set)
    ...
    for i, v in pairs(PresetGradients) do
        Tab:CreateButton({
            ...
            Callback = function()
                ...
                -- update UI (if supported)
                if c1cp and c1cp.Set then
                    c1cp:Set({Color = c1})
                end
                if c2cp and c2cp.Set then
                    c2cp:Set({Color = c2})
                end
                if c3cp and c3cp.Set then
                    c3cp:Set({Color = c3})
                end

                -- FORCE update (important because Set() may not fire callback)
                UpdateTheme()
            end,
        })
    end
